...
function hist(s) {
    const uCount = s.match(/u/ig) ? s.match(/u/ig).length : 0
    const wCount = s.match(/w/ig) ? s.match(/w/ig).length : 0
    const xCount = s.match(/x/ig) ? s.match(/x/ig).length : 0
    const zCount = s.match(/z/ig) ? s.match(/z/ig).length : 0

    const line = (s, count) => {
        return `${s}  ${count}${' '.repeat(6 - String(count).length)}${'*'.repeat(count)}`
    }
    let res = []
    if (uCount) res.push(line('u', uCount))
    if (wCount) res.push(line('w', wCount))
    if (xCount) res.push(line('x', xCount))
    if (zCount) res.push(line('z',zCount))
