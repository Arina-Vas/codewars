...
// x - временная переменная для мап, мап заменяет перебор элеметов массива, т.е. применяется сразу ко всему массиву, после стрелки пишем, что нужно сделать, в данном случае провести проверку(if, else) с помощью тернарного оператора и добавить соответсвующие значения в массив; если мы ничего не добавили, то массив д.б. пустым, поэтому [] в конце
